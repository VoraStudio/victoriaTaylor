...
function vtSplit(textEl, opts) {
  ...
  try {
    return new SplitText(textEl, opts);
  } catch (e) {
    return null;
  }
}
...
function initMobileMenu() {
  ...
  const mobileSplits = Array.from(document.querySelectorAll(".mobile-link .txt-original"))
    .map((link) => vtSplit(link, { type: "chars, lines", mask: "lines" }))
    .filter(Boolean);
...
function buildNavHoverTL(link, text) {
  ...
  if (splitOriginal && splitClone) {
    tl.to(splitOriginal.chars, { yPercent: -100, stagger: 0.02, duration: 0.4, ease: "power2.inOut" }).to(
      splitClone.chars,
      { yPercent: -100, stagger: 0.02, duration: 0.4, ease: "power2.inOut" },
      0,
    );
  }
  ...
  ? new Swiper(swiperEl, {
      ...
      on: {
        ...
        init: function () {
          ...
          gsap.fromTo(
            activeBg,
            { scale: 1.4, filter: "brightness(0.9)" },
            { scale: 1, duration: 2.5, ease: "power2.out", filter: "brightness(0.2)" },
          );
        },
        slideChangeTransitionEnd: function () {
          ...
          gsap.fromTo(
            activeBg,
            { scale: 0.75, filter: "brightness(0.2)" },
            {
              scale: 1,
              duration: 1.2,
              ease: "power4.out",
...
function hoverNextPrev(selector) {
  ...
  items.forEach((item) => {
    ...
    const tl = gsap.timeline({ paused: true });
    if (splitOriginal && splitClone) {
      tl.to(splitOriginal.chars, { yPercent: -100, stagger: 0.03, duration: 0.3, ease: "power2.inOut" }).to(
        splitClone.chars,
        { yPercent: -100, stagger: 0.03, duration: 0.3, ease: "power2.inOut" },
        0,
      );
    }
...
function initNoticiaEntrance() {
  ...
  let noticia;
  try {
    noticia = JSON.parse(noticiaData.textContent);
  } catch (e) {
    return;
  }
  if (!noticia) return;
...
function initArtistaEntrance() {
  ...
  /* ===== BIO ===== */
  var bioSplit = vtSplit(".artist-bio-text-p", { type: "lines", mask: "lines" });
  if (bioSplit && bioSplit.lines) {
...
const i18n = {
  es: {
    ...
    "hero.slide1.subtitle": "Arte Exclusivo",
    "hero.slide2.subtitle": "Talento Único",
    "hero.slide2.title": "Artistas Emergentes",
    "hero.slide3.subtitle": "Expresión Contemporánea",
    "hero.slide3.title": "Creatividad",
  ...
  ca: {
    ...
    "hero.slide1.subtitle": "Art Exclusiu",
    "hero.slide2.subtitle": "Talent Únic",
    "hero.slide2.title": "Artistes Emergents",
    "hero.slide3.subtitle": "Expressió Contemporània",
    "hero.slide3.title": "Creativitat",
  ...
  en: {
    ...
    "nosotros.hero.title": "Victoria Taylor",
    "nosotros.hero.text1":
      "Victoria Taylor is born as a meeting space between creation and appreciation. We are not a traditional gallery. We are a curatorial platform that selects, promotes and positions contemporary artists within an environment where their work can be seen, understood and valued. We believe in art as a form of universal language, capable of conveying identity, emotion and thought.",
    "nosotros.hero.text2":
      "We work with a clear vision: to give real value to artistic talent through careful selection and impeccable presentation.",
    ...
    "hero.slide1.subtitle": "Exclusive Art",
    "hero.slide2.subtitle": "Unique Talent",
    "hero.slide2.title": "Emerging Artists",
    "hero.slide3.subtitle": "Contemporary Expression",
    "hero.slide3.title": "Creativity",
...
function updateArtistLang(lang) {
  ...
  var a;
  try {
    a = JSON.parse(dataEl.textContent);
  } catch (e) {
    return;
  }
